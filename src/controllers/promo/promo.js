...
module.exports = {
  getAllPromos: async (req, res) => {
    try {
      ...
      res
        .status(200)
        .send(response(true, 'List of promos', result, { pageInfo }));
    } catch (e) {
    ...
    upload(req, res, async (fileError) => {
      ...
      if (!req.file) {
        ...
        if (promoValid.status) {
          ...
          res.status(400).send(response(true, promoValid.msg));
        }
      }
    });
  },
      ...
      if (!req.file) {
        ...
        if (promoExist) {
          ...
          if (promoValid.status) {
            ...
            try {
              const updatePromo = await promo.updatePromo([
                data,
                { id: parseInt(id) },
              ]);
              res
                .status(201)
                .send(response(true, promoValid.msg, updatePromo[0]));
            } catch (e) {
              res.status(500).send(response(false, e.message));
            }
          } else {
            res.status(400).send(response(false, promoValid.msg));
          }
        } else {
          res.status(404).send(response(false, promoExist));
        }
      }
    });
  },
